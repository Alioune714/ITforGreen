...
const Chatbot = () => {
  ...
  useEffect(() => {
    fetch('/data/udemy.json') // ✅ accessible depuis public/data/
      .then((res) => res.json())
      .then((json) => setCourses(json))
      .catch((err) => console.error('Erreur lors du chargement des formations :', err));
    
    setMessages([{ sender: 'chatbot', message: 'Bonjour 👋 Quels sont vos centres d’intérêt ?' }]);
  }, []);
  ...
  const handleMessageSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (result) {
        const carbone = (result.content_duration * 0.6).toFixed(2); // 🌱 estimation
        setMessages(prev => [
          ...prev,
          {
            sender: 'chatbot',
        ...
        setMessages(prev => [
          ...
          {
            sender: 'chatbot',
            message: 'Désolé 😔 je n’ai trouvé aucune formation correspondante. Essayez un autre mot-clé (ex : finance, trading, python...)'
          }
        ]);
      }
